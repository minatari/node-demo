...
//swiss army knife
//lets us read anything in static subdirectory (css, js), not above
...
app.get('/api/tasks', function(req, res, next) {
    //dont get any tasks already completed
    //rowid is automatically created w/ sqlite, primary key
...
app.post('/api/tasks', function(req, res, next) {
    ...
    db.run(sql, [newTask.title, newTask.done, newTask.createdOn], function(err) {
        ...
        //201 is http status code that everything is okay + something got created successfully
...
app.put('/api/tasks/:rowid', function(req, res, next) {
    ...
    db.run(Sql, [red.body.done, req.params.rowid], function(err) {
        if (err) {
            return next(err);
        }
